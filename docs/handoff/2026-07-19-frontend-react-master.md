...
fallback. The current independent artifact is `214,116 B` raw / `65,691 B`
...
and one six-action grant coordinate through the revision-checked
...
assignments, scope, flags and a hidden server field pass. Its production artifact is `208,250 B` raw /
`65,235 B` gzip / `56,291 B` Brotli. Authenticated Pilot acceptance completed
...
7. Migrate commands one vertical scope at a time. **Nomenclature, Component
   ...
   owner; Pilot write acceptance remains separate. Roles passport metadata and
   six-action grant toggles are locally complete through the `access-control`
   owner; assignments, scopes, read-only, active and Pilot write acceptance
