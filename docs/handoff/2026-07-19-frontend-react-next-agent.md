...
> создания исполняемого route/surface ledger и двух permanent read-only
> Pilot-срезов текущий доказанный прогресс — `50%`. План до настоящих
> `100%` зафиксирован в
...
- Текущий принятый Pilot checkpoint: `097d66c416ef61e091099c63b8bc272841c364f5`
  (`fix(weekly): preserve canonical Pilot row parity`) на ветке
  `codex/pilot-weekly-consolidation`; основная ветка миграции остаётся
  `codex/frontend-react-migration`.
...
- Historical Pilot read ledger: `21/24` принятых сценариев на разных
  релизах; current-release browser read на `.26` — `1/24` (Weekly).
  Diagnostics сохраняет историческое `.21` evidence и не считается свежей
  `.26` browser-проверкой.
...
- На `v.1.500.26-097d66c` без evaluation-флагов постоянно включены
...
- Active Pilot release is `v.1.500.21-8fb92d9`, immediate previous is
...
## Corrective Weekly consolidation acceptance 2026-07-21: release `.26`

This checkpoint supersedes the older `.21` block only as the statement of
current live state. The `.21` Diagnostics evidence remains historical and was
not rerun in the `.26` browser session.

- Active Pilot release is `v.1.500.26-097d66c` at exact commit
  `097d66c416ef61e091099c63b8bc272841c364f5`; immediate previous is
  `v.1.500.25-1f8369c`, pinned legacy is `v.1.500.18-93d02ed`.
...
- Weekly now builds its production read-model without a normal dependency on
  the legacy Weekly runtime. The legacy implementation remains lazy-loadable
  only through the explicit rollback selector.
- Authenticated `.26` browser acceptance reached React `ready`, `aria-busy=false`
  and exact `25 x 11`; every row text matched immutable `.25`. The live DOM/error
  state was clean. Live-console capture was unavailable and query-isolation was
  not repeated, so neither is claimed as fresh `.26` evidence; both remain
  separately covered by local production-shell QA.
...
The evidence-weighted global result is now **50%** with the exact vector
`14 / 18 / 9 / 2 / 2 / 5`. This checkpoint earns only two
legacy-consolidation points over the audited `48%` state; it does not close the
remaining global migration. Legacy rollback is preserved and Blueprint UI is
not used.
