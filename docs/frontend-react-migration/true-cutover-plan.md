...
Кандидат `v.1.500.49` расширяет тот же ускоренный cut без визуальной
полировки: current runtime больше не загружает legacy renderer/events для
...
visual QA для кандидата намеренно не выполнялись. До активации это только
implementation candidate и не повышает strict acceptance выше 50%.
