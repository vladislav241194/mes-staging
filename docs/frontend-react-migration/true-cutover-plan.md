...
Ветка принятого Weekly-среза: `codex/pilot-weekly-consolidation`
Аудитируемый immutable Pilot commit: `097d66c416ef61e091099c63b8bc272841c364f5`
...
Последующий аудит зависимостей показал, что прежнее утверждение о выводе Weekly
из legacy на `.19`/`.21` было преждевременным: React-рендерер уже был permanent,
но production read-model всё ещё строился через legacy runtime. Поэтому два
балла legacy-consolidation были отозваны, а точная исполняемая контрольная точка
перед текущим срезом составляла **48%**. Старые промежуточные формулировки
`49–50%` этой проверки не заменяют.

На `v.1.500.26-097d66c` Weekly Production Control получил самостоятельный
typed production read-model. После свежей аутентифицированной Pilot-проверки,
точного сравнения всех строк с `.25`, реального rollback
`.26 -> .25 -> .26` и финальной реактивации доказанная готовность составляет
**50%**. Прибавлены только два балла legacy-consolidation; functional parity,
Pilot acceptance, permanent runtime и quality controls не переоценивались.
Маркировка остаётся MOCK и в production-готовность не входит.

## Что проверено 2026-07-21

- Weekly consolidation прошёл локальный QA, immutable staging и Pilot activation.
- Pilot здоров по local и public health; активный release:
  `v.1.500.26-097d66c`, immediate previous — `v.1.500.25-1f8369c`,
  shared state — `ready`.
- Активный release привязан к commit
  `097d66c416ef61e091099c63b8bc272841c364f5`, source tree
...
- Историческая аутентифицированная приёмка Diagnostics остаётся привязанной к
  `.21`, а не выдаётся за свежую `.26`-проверку. На `.21` в desktop было проверено:
...
- Weekly проверен на `.26` в desktop: React `ready`, `25 x 11`, и текст каждой
  строки точно совпал с immutable `.25`. Live DOM/error state чистый; API
  live-console был недоступен, поэтому чистый live console не заявляется.
  Query-isolation на `.26` отдельно не повторялся; он остаётся локальным
  production-shell доказательством. Narrow приёмка исторически доказана на `.19`.
- Выполнен реальный previous-release rollback drill `.26 -> .25 -> .26`:
  после rollback и реактивации Weekly оставался точным `25 x 11` с тем же текстом
  строк. Pinned legacy release `v.1.500.18-93d02ed` разрешён dry-run-проверкой и
  содержит нулевой список React surfaces, но в этом drill не активировался.
...
- По накопленному журналу Pilot meaningful read evidence есть у 21/24
  ...
  Control — 1/24; Diagnostics остаётся историческим `.21` evidence.
...
| Strict QA, release и rollback-контроли | 5 | 5 | Полный QA/stage, immutable provenance, реальный previous-release rollback/reactivation и legacy dry-run доказаны |
...
`round(20 × 2/24) = 2`; legacy consolidation становится 2 только после `.26`,
потому что Weekly теперь не зависит от legacy read-model, а
Diagnostics не выводит из legacy весь верхнеуровневый
`productionStructureMatrix`. Pilot acceptance остаётся 9 баллов: fresh
current-release coverage составляет 1/24, а write coverage не изменилось и
...
Pilot reads, 1/24 current-release reads, 1/22 Pilot writes, две permanent
...
Статус 2026-07-21: для текущего Weekly-среза выполнен реальный drill
`.26 -> .25 -> .26`; финальный active release снова `.26`, а immediate previous
— `.25`. Pinned legacy `.18` в этом drill проверен только dry-run разрешением и
не активировался. Более ранний полный `.21` drill остаётся историческим
доказательством, но не подменяет текущую `.26` приёмку. Финальный drill всей
