...
ZIP/XML XLSX parser и 18,3 КБ legacy Directory modal/bind. Они загружаются
только за существующими lazy rollback/action boundaries и не попадают в normal React boot.
...
Ops checkpoint `v.1.500.42-c6208b8` активирован на Pilot с зелёным
local/public health. Dry-run на immediate previous `.41` прошёл. Dry-run на
pinned legacy `.18` также прошёл после контролируемого временного
отключения несовместимых command owners; все owners восстановлены в
`ON`. Реальный rollback не выполнялся, release pointer остался на `.42`.
