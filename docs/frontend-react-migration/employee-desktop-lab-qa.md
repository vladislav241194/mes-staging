...
- route, structure and PDF scopes return to legacy.
...
- remaining explicit legacy fallback scopes including structure and person switching;
- deviation guard, disabled flag, no overflow and clean console;
- independent entry `218,066 B` raw / `65,929 B` gzip under the unchanged
  `225,000 B / 68,000 B` production-entry budget;
- full twenty-four-scenario lab `523,720 B / 120,206 B` under its
  development-only `524,000 B / 125,000 B` budget;
...
The first local commit was `30.00 ms`, below the `2,000 ms` production-shell
gate. The bundled artifact is `210,409 B` raw / `65,468 B` gzip / `56,512 B`
