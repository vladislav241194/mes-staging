...
Locally complete user-managed create/edit/delete vertical scenario:

`open Directories -> Statuses -> inspect system lifecycle rows -> create one user-managed status -> edit it -> cancel deletion without writes -> confirm deletion -> verify removal through legacy`.
...
dispatch only typed `save-custom` and `delete-custom` commands; system statuses
...
Create assigns both markers in the production command boundary. Edit and
delete verify the existing stored row and stable ID. The owner also requires
the existing `directories:edit` RBAC permission. A forged marker cannot
convert or delete a system row, and denial of RBAC blocks an existing custom
row. No general Statuses editor was enabled in legacy.
...
- a disposable owner-only `0600` snapshot proves custom create/edit/delete,
  byte-identical cancel safety, removal persistence, stable authority,
  system-row immutability, legacy read-back and unchanged Planning
  routes/steps/slots;
- owner-level QA rejects system, forged, missing and RBAC-denied deletion while
  accepting exactly two valid custom writes and one valid custom deletion;
- the latest full scenario first commit was `19.40 ms`.

The independent entry is `213,503 B` raw / `65,173 B` gzip. The production
artifact is `207,032 B` raw / `64,632 B` gzip / `55,697 B` Brotli.

## Production boundary

Read-only activation remains protected by explicit runtime permissions and a
session request. Custom create/edit/delete has only the local
