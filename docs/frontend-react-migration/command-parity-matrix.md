...
board metadata has create/edit/delete, all nine existing-row BOM cell edits and
ID/table-bound row deletion.
Structure Employees,
...
| 15 | Roles and Access | Local complete: role label, description, default module, six-action grant toggles, role default scope including `self`, exact-employee immediate assignment replace/clear, guarded immediate second-role add and ID-bound deactivate/reactivate for unassigned roles; assigned/current-role deactivation and protected-contour access-control reset stay intentionally guarded, while durable effective-window, subject/assignment responsibility-scope and `readOnly` persistence remain owner/schema gaps rather than proven legacy parity | Critical | Separately gated Pilot metadata/grant/default-scope/assignment/unassigned-role lifecycle write evaluation with disposable coordinates and verified cleanup/reactivation; keep destructive reset outside React unless product policy explicitly changes; resolve each durable owner/schema contract separately |
...
| 22 | Authorization | React TS complete: raw PostgreSQL System Domains build the picker without `auth_render`; PIN uses the signed employee-session owner, exact active-person revalidation and actor-bound elevation | Critical | Current-release Pilot PIN lifecycle and rollback/reactivation acceptance remain deferred |
...
Boards/BOM now has local metadata create/edit/delete, all nine existing-row
cell edits and row-deletion parity through the existing lazy Products command owner. Production-shell QA
...
unrelated rows, then exercises every non-quantity A:I field through the same
owner. It proves package normalization `805 -> 0805`, existing/new keyed
Nomenclature semantics, full-row owner read-back and the complete nine-value
legacy row. Row deletion uses an
ID/table-bound confirmation, rejects a changed full-row snapshot, delegates to
`deleteBomImportRow`, preserves independently addressable Nomenclature and reads
the exact three-row result through legacy. It also preserves hidden fields,
`projectId` and imported rows on edit, synchronizes both existing and
