...
`scripts/ui-raw-token-audit.mjs` пока работает в warning-only режиме. Его задача сейчас - показать, где еще остались прямые visual values, а не остановить сборку.
