...
- `projectId` остается только техническим alias для `specificationId` в маршрутах, партиях, слотах Ганта и старых сохранениях localStorage;
...
- `projectId` в сохраненных route/batch/slot объектах;
...
- Заказ на производство автоматически создает партии.
...
| Заказ/партия | `planningState.batches` | Партийность производства | Генерируется из маршрута и количества |
| Операция в Ганте | `planningState.slots` | Конкретный запланированный слот | Ссылается на маршрут, партию, шаг маршрута, участок |
...
erDiagram
  ...
  ROUTE ||--o{ BATCH : routeId
  ROUTE_STEP ||--o{ SLOT : routeStepId
  BATCH ||--o{ SLOT : batchId
...
flowchart LR
  ...
  E --> F["Заказ на пр-во<br/>количество, партии, платы в мультиплате"]
...
3. Сконструировать партийность.
4. Указать платы в мультиплате для печатных плат.
5. Передать в Гант.
...
- Если партий нет, система создает партию автоматически.
- Изменение количества маршрута может пересчитать партии и слоты.
...
- Слоты создаются из `routeSteps * batches`.
...
- Стрелки зависимостей строятся из порядка routeSteps внутри партии и задачи спецификации.
...
- `projectId` в `routes`, `batches`, `slots`, `bomLists`, `specifications`, `calculatorState`.
...
flowchart TB
  ...
  DEL_SPEC --> DEL_BATCHES["Удалить партии"]
...
| Передать в заказ | Может создаться production context и партия | Нет отдельного объекта "проект", но задание появляется |
| Передать в Гант | Создаются слоты по всем партиям и шагам | Операций становится много |
| Загрузка/нормализация planningState | Добавляется складской шаг/слот | Система сама добавляет финальную операцию |
| Удалить спецификацию | Удаляются маршруты, партии и слоты | Каскад большой |
...
2. **Два состояния: `directoryState` и `planningState`**  
   Спецификации/BOM лежат в одном состоянии, маршруты/партии/слоты в другом. Любая связь между ними требует ручной синхронизации.
...
- Заказ на производство отвечает на вопрос "сколько и какими партиями производим".
