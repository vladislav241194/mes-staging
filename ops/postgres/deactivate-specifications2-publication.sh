...
verify_active_release_contract || { echo "Active release provenance or manifest-bound Specifications 2.0 publication contract is invalid." >&2; exit 1; }
...
for attempt in $(seq 1 12); do
  if readiness="$(request_readiness 2>/dev/null)" \
    && node "${APP_DIR}/scripts/specifications2-rollout-readiness-policy.mjs" publication-disabled "$readiness"; then
    completed=1
    break
  fi
  sleep 1
...
[[ $completed -eq 1 ]] || { echo "Publication capability remained enabled or could not be proved disabled; prior service configuration will be restored." >&2; exit 1; }
