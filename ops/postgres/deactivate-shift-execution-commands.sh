#!/usr/bin/env bash
# Reversible Shift Execution command deactivation for a compatible active release.
set -Eeuo pipefail
...
backup_dir="$(mktemp -d /root/.mes-shift-execution-command-rollback.XXXXXX)"
had_previous=0
applied=0
completed=0
restore_on_failure() {
...
[[ $completed -eq 1 ]] || { echo "Shift Execution commands remained enabled or PostgreSQL authority could not be proved; prior configuration will be restored." >&2; exit 1; }

echo "Shift Execution commands are disabled; PostgreSQL data and authority are preserved."
