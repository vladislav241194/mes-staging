...
DROPIN_FILE="${DROPIN_DIR}/79-react-employee-desktop-evaluation.conf"
SOURCE_FILE="${APP_DIR}/ops/frontend/mes-pilot-react-employee-desktop-evaluation.conf"
backup_dir="$(mktemp -d /root/.mes-react-employee-desktop-evaluation.XXXXXX)"
had_previous=0
...
grep -Fq 'Environment=MES_REACT_EMPLOYEE_DESKTOP=1' "$SOURCE_FILE" \
  || { echo "Feature flag is missing from $SOURCE_FILE" >&2; exit 1; }
grep -Fq 'Environment=MES_REACT_EMPLOYEE_DESKTOP_READ_ONLY_EVALUATION=1' "$SOURCE_FILE" \
...
for attempt in $(seq 1 12); do
  ...
  if grep -Fq '"status":"ok"' <<<"$health" \
    && grep -Fq '"MES_REACT_EMPLOYEE_DESKTOP":true' <<<"$home" \
    && grep -Fq '"MES_REACT_EMPLOYEE_DESKTOP_READ_ONLY_EVALUATION":true' <<<"$home"; then
    completed=1
    break
  fi
  sleep 1
...
[[ $completed -eq 1 ]] \
  || { echo "React Employee Desktop evaluation did not become ready; prior service configuration will be restored." >&2; exit 1; }

echo "Session-scoped read-only React Employee Desktop evaluation is permitted."
