#!/usr/bin/env bash
# Permit an explicitly requested, pre-PIN, read-only React evaluation.
set -euo pipefail
...
DROPIN_FILE="${DROPIN_DIR}/88-react-auth-picker-evaluation.conf"
SOURCE_FILE="${APP_DIR}/ops/frontend/mes-pilot-react-auth-picker-evaluation.conf"
backup_dir="$(mktemp -d /root/.mes-react-auth-picker-evaluation.XXXXXX)"
had_previous=0
...
for attempt in $(seq 1 12); do
  ...
  if grep -Fq '"status":"ok"' <<<"$health" \
    && grep -Fq '"MES_REACT_AUTH_PICKER":true' <<<"$home" \
    && grep -Fq '"MES_REACT_AUTH_PICKER_READ_ONLY_EVALUATION":true' <<<"$home"; then completed=1; break; fi
  sleep 1
done
[[ $completed -eq 1 ]] || { echo "React Authorization picker evaluation did not become ready; prior configuration will be restored." >&2; exit 1; }
echo "Session-scoped read-only React Authorization picker evaluation is permitted."
