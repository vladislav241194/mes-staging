...
DROPIN_FILE="/etc/systemd/system/${SERVICE}.service.d/70-react-nomenclature-evaluation.conf"
backup_dir="$(mktemp -d /root/.mes-react-nomenclature-deactivation.XXXXXX)"
had_previous=0
...
[[ $completed -eq 1 ]] \
  || { echo "React Nomenclature evaluation did not turn off cleanly; prior service configuration will be restored." >&2; exit 1; }

echo "React Nomenclature evaluation is disabled; every session uses legacy."
