#!/usr/bin/env bash
# Controlled Directory Nomenclature Types and Boards/BOM command activation.
set -Eeuo pipefail
...
DROPIN_FILE="${DROPIN_DIR}/50-directory-cluster-commands.conf"
SOURCE_FILE="${APP_DIR}/ops/shared-state/mes-pilot-directory-cluster-commands.conf"
...
verify_active_release_contract() {
  ...
  /usr/bin/node "${active_target}/scripts/release-server-command-contract-verify.mjs" \
    ...
    --expected-release-id="$release_id" --contract=directory-cluster >/dev/null
}
...
[[ -f "$SOURCE_FILE" ]] || { echo "Missing Directory Cluster drop-in source." >&2; exit 1; }

other_dropins="$(grep -RIl -E 'MES_ENABLE_DIRECTORY_CLUSTER_SERVER_COMMANDS=1' "$DROPIN_DIR" 2>/dev/null | grep -Fvx "$DROPIN_FILE" || true)"
[[ -z "$other_dropins" ]] || { echo "An unmanaged Directory Cluster owner is configured:" >&2; printf '%s\n' "$other_dropins" >&2; exit 1; }
...
backup_dir="$(mktemp -d /root/.mes-directory-cluster-command-activation.XXXXXX)"
had_previous=0
applied=0
completed=0
restore_on_failure() {
  ...
  if [[ "$main_pid" =~ ^[1-9][0-9]*$ ]] \
    ...
    && /usr/bin/node -e '
      for (const source of process.argv.slice(1)) {
        ...
        if (payload?.ok !== true || payload?.capabilities?.serverCommandsConfigured !== true) process.exit(1);
      }
    ' "$types" "$boards"; then completed=1; break; fi
  sleep 1
done
[[ $completed -eq 1 ]] || { echo "Directory Cluster command ownership could not be proved enabled; prior configuration will be restored." >&2; exit 1; }
echo "Directory Cluster server commands are enabled on the manifest-verified active release."
