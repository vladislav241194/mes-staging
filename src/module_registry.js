...
const CORE_MODULE_BLUEPRINTS = [
  coreBlueprint({
    ...
    regression: { type: "special-runtime-protected", hasTable: false, hasActions: true, hasGantt: true, allowedInternalOverflowSelectors: [".gantt-shell", ".planner-workspace"], requiredSelectors: [".gantt-shell", ".timeline-row", ".rows-layer", ".operation-slot", "[data-ui-component='GanttToolbar']"], futurePhase: "Phase 6 Gantt drag/resize/dependency routing interaction depth" },
    overlayProbeException: "Protected by the dedicated Gantt interaction and guardrail suites.",
    mobileLimitedReason: "Gantt is an absolute-positioned timeline; narrow smoke checks blank/runtime/guardrails, not full mobile UX.",
    ...
    capabilities: { actions: true, overlays: ["slot-editor", "optimization", "split"] },
    sourceFiles: ["src/modules/gantt_runtime/render.js"],
  }),
  coreBlueprint({
  ...
  coreBlueprint({
    ...
    capabilities: { table: true, tree: true, actions: true }, sourceFiles: ["src/modules/specifications2/render.js"],
  }),
  coreBlueprint({
