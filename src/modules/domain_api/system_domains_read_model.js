...
export function createSystemDomainsReadModel({ fetchImpl = globalThis.fetch, url = "/api/v1/system-domains", now = () => Date.now() } = {}) {
  const state = { item: null, etag: "", fetchedAt: -1, loading: null, error: "" };
  async function refresh({ force = false } = {}) {
    if (!force && state.fetchedAt >= 0 && now() - state.fetchedAt < MAX_AGE_MS) return { ok: true, changed: false, notModified: true, item: state.item };
    ...
    state.loading = (async () => {
      try {
        ...
        if (response.status === 304) {
          ...
          return { ok: true, changed: false, notModified: true, item: state.item };
        }
        ...
        return { ok: true, changed, notModified: false, item: state.item, revision: payload.revision };
      } catch (error) {
  ...
  return { refresh, get: () => ({ item: state.item, etag: state.etag, fetchedAt: state.fetchedAt, error: state.error }) };
}
