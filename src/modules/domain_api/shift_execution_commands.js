...
export function createShiftExecutionCommands({
  ...
  let capability = { enabled: false, primaryPostgres: false, schemaReady: false, fetchedAt: 0, loading: null, error: "" };
  ...
  async function refreshCapability({ force = false } = {}) {
    ...
    capability.loading = (async () => {
      try {
        ...
        const changed = next.enabled !== capability.enabled || next.primaryPostgres !== capability.primaryPostgres || next.schemaReady !== capability.schemaReady;
        capability = next;
