...
export function createSpecifications2ProductionOwner({
  ...
  const getContext = () => {
    ...
    return { store, selected, revisionState, capability };
  };

  const getPayload = () => {
    const { store, selected, revisionState, capability } = getContext();
  ...
  const execute = async (command = {}) => {
    ...
    if (!routeIds(revision).has(routeSourceDraftId)) {
      return { ok: false, message: "Маршрут больше не входит в опубликованную ревизию." };
    }
    if (!Number.isSafeInteger(quantity) || quantity < 1 || quantity > 9_999_999) {
      return { ok: false, message: "Количество должно быть целым положительным числом." };
    }
    ...
    try {
      ...
      return {
        ok: true,
        id: text(result.item?.id || result.workOrder?.id),
  ...
  return { getContext, getPayload, selectEntry, execute };
}
