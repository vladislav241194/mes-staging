...
export function createAuthPickerReactIslandHost({ getActivation, getPayload, getTargetRoot, requestLegacyRender, reportError = (error) => console.error("[MES] Authorization picker React island failed", error) } = {}) {
  return createReactIslandHost({
    ...
    getIneligibilityReason: (activation) => {
      ...
      if (activation.accessMode !== "read-only-evaluation") return "security-parity-incomplete";
      return "";
    },
    loadIsland: async () => {
    ...
    mountIsland: ({ loadedIsland, target, payload, onError, onReady, onRequestLegacy }) => loadedIsland.mountAuthPickerReactIsland(target, payload, { onError, onReady, onRequestLegacy }),
  });
}
