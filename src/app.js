...
let cancelAuthPrototypePinFeedback, createSpekiSpecification, ensureNomenclatureTypeExists, getActiveSpecificationForModule, getAuthPrototypePeople, getBomImportRows, getBomList, getBomResultNomenclatureItem, getDefaultSmtLineConfigurations, getDirectoryRows, getFallbackNomenclatureType, getNomenclatureDeleteUsage, getNomenclatureItem, getResourceBaseCph, getResourcesForWorkCenter, getRouteBindingContext, getRouteBindingModeForSelection, getRouteBomList, getRouteDocumentKind, getRouteDocumentKindLabel, getRouteDocumentKindShortLabel, getRouteLineageSubjectName, getRouteModuleSelectionName, getRouteModuleSelectionValue, getRouteRootRoute, getRouteScopeRootTask, getRouteSpecification, getRoutesForModule, getSlotGanttResourceId, getSlotGanttWorkCenterId, getSmtLineConfigurations, getSmtLineNumberFromText, getSpecificationBomEntries, getSpecificationById, getSpecificationItemBomId, getSpekiStructureItemDisplayName, getSpekiStructureItemLabel, getSpekiStructureTableRows, importBomFromXlsxFile, isAuthPrototypePinFeedbackLocked, migrateSpecificationBomRowsToNomenclature, normalizeBomImportRow, normalizeLookupText, normalizeNomenclatureType, normalizeRouteBindingValue, resolveRouteModuleProjectId, syncNomenclatureTypesFromItems, syncSpecificationDerivedFields, upsertBomResultToNomenclature;
...
function getShiftMasterBoardModel() {
  return shiftMasterBoardCommandOwner.getModel();
}
...
function getShiftWorkOrdersReactActivation() {
  ...
  const runtimeActivation = resolveReactRuntimeActivation({
    ...
    evaluationFeatureEnabled: MES_RUNTIME_CONFIG.MES_REACT_SHIFT_WORK_ORDERS === true && serverEvaluationAllowed,
...
const shiftWorkOrdersReactIslandHost = createShiftWorkOrdersReactIslandHost({
  ...
  executeCommand: async (command = {}) => {
    const localQa = getShiftWorkOrdersReactLocalQaOverrides();
    const activation = getShiftWorkOrdersReactActivation();
    if ((activation.accessMode !== "react" && !localQa.writeEvaluation) || shiftExecutionServerState.commandsEnabled !== true) return { ok: false, message: "Изменения Журнала СЗН в React недоступны." };
...
function initializeProductsCompatibilityRuntime() {
  ({
    cancelAuthPrototypePinFeedback,
  ...
  } = createProductsCompatibilityRuntime({
  BOARD_SPEC_TERM,
...
function hydrateShiftExecutionServerProjection() {
  ...
  void ensureShiftExecutionDomainApiModule().then((ready) => ready
    ...
    if (ui?.activeModule === "shiftWorkOrders") {
      window.setTimeout(() => {
...
initializeProductsCompatibilityRuntime();
...
appEventsService = createAppEventsServiceModule({
  ...
  saveShiftMasterBoardAssignment: (rowId, assignment = {}) => {
    const result = shiftMasterBoardCommandOwner.execute({
      type: "save-assignment",
      rowId,
      executors: assignment.executors,
    });
    return result?.ok === true ? result.assignment : null;
  },
