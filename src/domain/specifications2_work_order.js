...
export function buildSpecifications2WorkOrderCommand({ revision = {}, route = {}, operations = [], quantity, idempotencyKey }) {
  ...
  const seed = `${revisionId}:${routeId}:${key}`;
  const workOrderId = stableId("wo-spec2", seed);
  const operationRows = operations.map((operation, index) => {
    ...
    return {
      id: stableId("woo-spec2", `${workOrderId}:${operation.id || operation.source_operation_id || index + 1}`),
      workOrderId,
      operationId,
      name,
      workCenterId,
      nextWorkCenterId: clean(operation.next_work_center_id || operation.nextWorkCenterId),
      sequenceNo: Number(operation.sequence_no || operation.sequenceNo || index + 1),
  ...
  return {
    workOrder: {
      id: workOrderId,
      number: `WO-S2-${Number(revision.revision_no || revision.revisionNo || 1)}-${workOrderId.slice(-8).toUpperCase()}`,
    ...
    source: { specificationRevisionId: revisionId, routeDocumentId: routeId, idempotencyKey: key },
  };
}
