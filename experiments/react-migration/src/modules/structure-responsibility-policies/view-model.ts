...
export function buildResponsibilityPolicyRegistryOptions(model: StructureResponsibilityPoliciesReadModel) { return STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => ({ ...definition, count: model.counts[definition.id], action: definition.id === "responsibilityPolicies" ? "responsibilityPolicies" as const : "navigate" as const })); }
