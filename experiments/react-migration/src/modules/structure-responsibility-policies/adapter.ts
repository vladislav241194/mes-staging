import { formatStructurePersonName, resolveStructureMigrationDiagnosticsCount, STRUCTURE_REGISTRY_DEFINITIONS, type StructureRegistryCount, type StructureRegistryId } from "../structure-employees/adapter";
...
export interface StructureResponsibilityPoliciesReadModel { policies: StructureResponsibilityPolicy[]; counts: Record<StructureRegistryId, StructureRegistryCount>; employees: StructureResponsibilityEmployeeOption[]; canCreateEdit: boolean; canArchive: boolean; canElevate?: boolean; writeUnavailableReason?: string }
...
export function adaptStructureResponsibilityPolicies(payload: unknown): StructureResponsibilityPoliciesReadModel {
  ...
  return { policies, counts: Object.fromEntries(STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => [definition.id, definition.id === "migrationDiagnostics" ? diagnosticsCount : rows(registries[definition.id]).length])) as Record<StructureRegistryId, StructureRegistryCount>, employees: employees.flatMap((employee) => text(employee.id) ? [{ id: text(employee.id), label: label(employee) }] : []).sort((left, right) => left.label.localeCompare(right.label, "ru") || left.id.localeCompare(right.id, "en")), canCreateEdit: capabilities.createEdit === true, canArchive: capabilities.archive === true, canElevate: capabilities.employeeElevation === true, writeUnavailableReason: text(capabilities.writeUnavailableReason) };
}
