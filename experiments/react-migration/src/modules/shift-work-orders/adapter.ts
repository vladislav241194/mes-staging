...
function adaptExecutor(value: unknown, index: number): ShiftWorkOrderExecutor | null {
  ...
  return name ? { id, name, quantity: number(source.quantity), note: text(source.note) } : null;
}
...
function adaptRow(value: unknown): ShiftWorkOrderRow | null {
  ...
  return {
    ...
    resourceLabel: text(source.resourceLabel), masterName: text(source.masterName, "Мастер не назначен"),
