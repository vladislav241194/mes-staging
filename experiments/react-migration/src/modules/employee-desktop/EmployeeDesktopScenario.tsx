...
export function EmployeeDesktopScenario({ payload, onCommand, onRequestLegacy }: { payload: unknown; onCommand?(command: EmployeeDesktopReactCommand): Promise<{ ok?: boolean; message?: string } | void>; onRequestLegacy?(scope?: string): void }) {
  ...
  useEffect(() => { setActualQuantity(String(selected?.actualQuantity ?? 0)); setDefectQuantity(String(selected?.defectQuantity ?? 0)); setDeviationComment(""); setCommandError(""); }, [selected?.id, selected?.actualQuantity, selected?.defectQuantity]);
  ...
  return <OperationalPage className="employee-desktop-react" label="Рабочий стол">
    <ModuleHeader eyebrow="Оперативное управление" title="Рабочий стол" badge={<span className="lab-badge">{model.canStartTask || model.canSaveFact ? "React · task/fact evaluation" : "read-only React"}</span>} />
