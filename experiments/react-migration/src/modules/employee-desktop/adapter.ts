...
export interface EmployeeDesktopTask {
  ...
  previousOperation: string; nextOperation: string;
}
...
export interface EmployeeDesktopModel {
  ...
  personName: string; roleLabel: string; canViewAll: boolean; canStartTask: boolean; canSaveFact: boolean; assignedQuantity: number; goodQuantity: number; activeCount: number; doneCount: number;
}

function adaptTask(value: unknown): EmployeeDesktopTask | null {
...
export function adaptEmployeeDesktopPayload(payload: unknown): EmployeeDesktopModel {
  const source = record(payload); const model = record(source.model); const capabilities = record(source.capabilities); const tasks = list(model.tasks).map(adaptTask).filter(Boolean) as EmployeeDesktopTask[]; const selectedId = text(record(model.selectedTask).id); const person = record(model.person); const role = record(model.role);
  ...
  return { tasks, selectedTask: tasks.find((task) => task.id === selectedId) || tasks[0] || null, people, viewedPersonId: text(model.viewedPersonId, "__all"), personName: personName(person.name), roleLabel: text(role.label || role.name, "Исполнитель"), canViewAll: model.canViewAll === true, canStartTask: capabilities.taskStart === true, canSaveFact: capabilities.factSave === true, assignedQuantity: number(model.assignedQuantity), goodQuantity: number(model.goodQuantity), activeCount: list(model.activeTasks).length, doneCount: list(model.doneTasks).length };
}
