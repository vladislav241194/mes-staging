...
export interface EmployeeDesktopModel {
  ...
  personName: string; roleLabel: string; canViewAll: boolean; canStartTask: boolean; assignedQuantity: number; goodQuantity: number; activeCount: number; doneCount: number;
}
...
export function adaptEmployeeDesktopPayload(payload: unknown): EmployeeDesktopModel {
  ...
  return { tasks, selectedTask: tasks.find((task) => task.id === selectedId) || tasks[0] || null, people, viewedPersonId: text(model.viewedPersonId, "__all"), personName: personName(person.name), roleLabel: text(role.label || role.name, "Исполнитель"), canViewAll: model.canViewAll === true, canStartTask: capabilities.taskStart === true, assignedQuantity: number(model.assignedQuantity), goodQuantity: number(model.goodQuantity), activeCount: list(model.activeTasks).length, doneCount: list(model.doneTasks).length };
}
