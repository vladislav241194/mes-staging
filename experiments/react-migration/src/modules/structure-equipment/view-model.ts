...
export function buildEquipmentRegistryOptions(model: StructureEquipmentReadModel) { return STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => ({ ...definition, count: model.counts[definition.id], action: definition.id === "equipment" ? "equipment" as const : "navigate" as const })); }
