...
export interface StructureEquipmentReadModel { equipment: StructureEquipmentItem[]; counts: Record<StructureRegistryId, StructureRegistryCount>; orgUnits: StructureEquipmentOption[]; workCenters: StructureEquipmentOption[]; scheduleTemplates: StructureEquipmentOption[]; canCreateEdit: boolean; canArchive: boolean; canElevate?: boolean; writeUnavailableReason?: string }
...
export function adaptStructureEquipment(payload: unknown): StructureEquipmentReadModel {
  ...
  return { equipment, counts: Object.fromEntries(STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => [definition.id, definition.id === "migrationDiagnostics" ? diagnosticsCount : rows(registries[definition.id]).length])) as Record<StructureRegistryId, StructureRegistryCount>, orgUnits: options(orgUnitRows), workCenters: options(workCenterRows), scheduleTemplates: options(scheduleRows), canCreateEdit: capabilities.createEdit === true, canArchive: capabilities.archive === true, canElevate: capabilities.employeeElevation === true, writeUnavailableReason: text(capabilities.writeUnavailableReason) };
}
