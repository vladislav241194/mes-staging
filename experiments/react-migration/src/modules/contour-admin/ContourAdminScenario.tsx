...
export function ContourAdminScenario({ payload, onCommand }: { payload: unknown; onCommand?(command: ContourAdminReactCommand): Promise<CommandResult | void> }) {
  ...
  return <OperationalPage className="contour-admin-react" label="Контуры">
    <ModuleHeader eyebrow="Система" title="Контуры" badge={<span className="lab-badge">{model.canExecuteOps ? "React TS · protected Ops" : "React TS · read-only"}</span>} />
    ...
    <Panel heading={<div className="panel-heading"><div><p>Защищённый Ops API</p><h2>Сценарии управления</h2></div><StatusToken label={model.canExecuteOps ? "admin RBAC" : "команды недоступны"} tone="warning" /></div>}><TableWrap><table className="scenario-table"><thead><tr><th>Сценарий</th><th>Направление</th><th>Ответственный</th><th>Риск</th><th>Статус</th><th>Действие</th></tr></thead><tbody>{model.scenarios.map((item) => <tr data-contour-admin-scenario={item.id} key={item.id}><td><strong>{item.label}</strong><small>{item.result}</small></td><td>{item.source} → {item.target}</td><td>{item.owner}</td><td><StatusToken label={item.risk} tone={item.tone} /></td><td>{item.status}{item.commands.map((command) => results[command.id] ? <small className={results[command.id].ok ? "is-ok" : "is-error"} data-contour-admin-command-result={command.id} key={command.id}>{results[command.id].message || (results[command.id].ok ? "Выполнено" : "Ошибка")}</small> : null)}</td><td><div className="contour-admin-react-actions">{item.commands.length ? item.commands.map((command) => <ActionButton disabled={!model.canExecuteOps} key={command.id} onClick={() => openCommand(item, command)} title={model.canExecuteOps ? "Открыть подтверждение защищённой команды" : "Ops API недоступен текущей сессии"} variant="secondary">{command.label}</ActionButton>) : <ActionButton disabled title="Для действия ещё нет серверного владельца" variant="secondary">Недоступно</ActionButton>}</div></td></tr>)}</tbody></table></TableWrap></Panel>
