...
export function ContourAdminScenario({ payload, onCommand, onRequestLegacy }: { payload: unknown; onCommand?(command: ContourAdminReactCommand): Promise<CommandResult | void>; onRequestLegacy?(scope?: string): void }) {
  ...
  const openCommand = (scenario: Scenario, command?: ContourAdminCommand) => {
    if (!model.canExecuteOps || !command) { onRequestLegacy?.(`action:${scenario.id}`); return; }
