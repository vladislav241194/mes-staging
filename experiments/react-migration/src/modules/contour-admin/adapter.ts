import { buildContourAdminProductionModel } from "./production-model";
...
export function adaptContourAdminPayload(payload: unknown) {
  const root = record(payload); const model = record(root.model || (Object.hasOwn(root, "capabilities") ? buildContourAdminProductionModel() : payload)); const capabilities = record(root.capabilities);
