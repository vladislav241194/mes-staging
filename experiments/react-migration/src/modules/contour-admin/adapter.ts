...
export interface ContourAdminScenario { id: string; label: string; source: string; target: string; owner: string; risk: string; status: string; result: string; tone: "success" | "warning" | "neutral"; actions: number }
...
export function adaptContourAdminPayload(payload: unknown) {
  ...
  const scenarios = list(model.scenarios).map((value, index): ContourAdminScenario => { const item = record(value); return { id: text(item.id, `scenario-${index}`), label: text(item.label, "Сценарий"), source: text(item.source), target: text(item.target), owner: text(item.owner), risk: text(item.risk), status: text(item.status), result: text(item.result), tone: tone(item.tone), actions: item.precheckActionId ? 2 : 1 }; });
  ...
  return { contours, scenarios, speedRows, guardrails: list(model.guardrails).map((value) => text(value)).filter(Boolean) };
}
