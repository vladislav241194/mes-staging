...
export interface StructurePosition { id: string; name: string; code: string; kind: string; kindLabel: string; orgUnitId: string; orgUnitLabel: string; workCenterId: string; workCenterLabel: string; scheduleTemplateId: string; scheduleTemplateLabel: string; statusLabel: string; statusTone: "success" | "warning" }
export interface StructurePositionsReadModel { positions: StructurePosition[]; counts: Record<StructureRegistryId, number> }
...
export function adaptStructurePositions(payload: unknown): StructurePositionsReadModel {
  ...
  return { positions, counts: Object.fromEntries(STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => [definition.id, definition.id === "migrationDiagnostics" ? diagnosticsCount : rows(registries[definition.id]).length])) as Record<StructureRegistryId, number> };
}
