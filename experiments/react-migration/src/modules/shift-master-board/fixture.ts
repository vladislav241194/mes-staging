...
export const shiftMasterBoardFixture = { model, capabilities: { assignmentSave: true, factSave: true, laneMove: true } };
...
export const shiftMasterBoardUpdateFixture = { model: { ...model, rows: updatedRows, lanes: lanes.map((item) => ({ ...item, rows: item.rows.map((row) => updatedRows.find((entry) => entry.id === row.id) || row) })), selectedRow: updatedRows[1], assignedQuantity: 260 }, capabilities: { assignmentSave: true, factSave: true, laneMove: true } };
...
export function createShiftMasterBoardFactFixture(rowId: string, fact: { actualQuantity: number; defectQuantity: number; laborMinutes: number; executorCount: number; comment: string; deviationComment: string }) {
  ...
  return { model: { ...model, rows: nextRows, lanes: lanes.map((laneItem) => ({ ...laneItem, rows: nextRows.filter((row) => row.boardLaneId === laneItem.id) })), selectedRow: nextRows.find((row) => row.id === rowId) || nextRows[0], factQuantity: nextRows.reduce((sum, row) => sum + row.boardGoodQuantity, 0), openQuantity: nextRows.reduce((sum, row) => sum + Math.max(0, row.plannedQuantity - row.boardGoodQuantity), 0) }, capabilities: { assignmentSave: true, factSave: true, laneMove: true } };
}
export function createShiftMasterBoardLaneFixture(rowId: string, laneId: string) {
  const nextRows = rows.map((row) => row.id === rowId ? { ...row, boardLaneId: laneId } : row);
  return { model: { ...model, rows: nextRows, lanes: lanes.map((laneItem) => ({ ...laneItem, rows: nextRows.filter((row) => row.boardLaneId === laneItem.id) })), selectedRow: nextRows.find((row) => row.id === rowId) || nextRows[0] }, capabilities: { assignmentSave: true, factSave: true, laneMove: true } };
}
export function createShiftMasterBoardCarryoverFixture(dateKey: string, carryoverId: string) {
  ...
  return { model: { ...model, window: { label: "20.07.2026 · дневная смена" }, dateKey, rows: [carryover], lanes: lanes.map((laneItem) => ({ ...laneItem, rows: laneItem.id === "intake" ? [carryover] : [] })), selectedRow: carryover, plannedQuantity: 24, assignedQuantity: 0, factQuantity: 0, openQuantity: 24 }, capabilities: { assignmentSave: true, factSave: true, laneMove: true } };
}
