...
export function RolesScenario({ payload, onCommand }: {
  ...
  const addAssignmentBlocked = Boolean(
    advancedAssignmentDraft.assignmentId
    || advancedAssignmentDraft.validFrom
    || advancedAssignmentDraft.validTo
    || advancedAssignmentDraft.responsibilityScope.type !== "factory"
    || advancedAssignmentDraft.responsibilityScope.targetId
    || advancedEmployee?.assignments.some((assignment) => assignment.roleId === advancedAssignmentDraft.roleId)
    || advancedEmployee?.assignments.some((assignment) => !assignment.id),
  );
  ...
  const header = <ModuleHeader eyebrow="Система · System Domains" title="Роли и доступ" badge={<span className="lab-badge" data-react-parity-status="partial">{model.canEditMetadata ? "React · owner writes ready · partial" : "React · только чтение · partial"}</span>} />;
  const sidebar = (
    ...
    <ModulePage header={header} sidebar={sidebar}>
      <section className="workspace-main">
        {selected ? <>
          ...
          <Panel heading={<div className="panel-heading"><div><h2>Расширенные назначения · React</h2><p>Немедленное добавление роли owner-backed; период действия и scope остаются fail-closed.</p></div><StatusToken label={model.deferredCapabilities.canEditMultipleAssignments ? "add owner ready" : "fail-closed"} tone={model.deferredCapabilities.canEditMultipleAssignments ? "success" : "warning"} /></div>}>
            <div className="react-nomenclature-editor" data-react-roles-deferred-controls>
              ...
              <label><span>Назначение</span><select data-react-role-advanced-assignment disabled={!advancedEmployee?.assignments.length} onChange={(event) => { const assignment = advancedEmployee?.assignments.find((item) => item.id === event.currentTarget.value); setAdvancedAssignmentDraft((current) => assignment ? { ...current, assignmentId: assignment.id, roleId: assignment.roleId, validFrom: assignment.validFrom, validTo: assignment.validTo, responsibilityScope: assignment.responsibilityScope || current.responsibilityScope } : { ...current, assignmentId: "", validFrom: "", validTo: "", responsibilityScope: { type: "factory", targetId: "" } }); }} value={advancedAssignmentDraft.assignmentId}><option value="">Новое назначение</option>{advancedEmployee?.assignments.map((assignment, index) => <option disabled={!assignment.id} key={assignment.id || index} value={assignment.id}>{model.roles.find((role) => role.id === assignment.roleId)?.label || assignment.roleId}{assignment.id ? "" : " · нет stable ID"}</option>)}</select></label>
              ...
              <div className="react-nomenclature-editor-actions">
                <button className="action action--primary" data-react-role-add-assignment disabled={model.deferredCapabilities.canEditMultipleAssignments !== true || !advancedAssignmentDraft.employeeId || !advancedAssignmentDraft.roleId || addAssignmentBlocked || Boolean(deferredSavingType)} onClick={() => void runDeferredCommand({ type: "add-assignment", payload: { employeeId: advancedAssignmentDraft.employeeId, confirmEmployeeId: advancedAssignmentDraft.employeeId, expectedAssignmentIds: (advancedEmployee?.assignments || []).map((assignment) => assignment.id).sort(), roleId: advancedAssignmentDraft.roleId, validFrom: "", validTo: "", responsibilityScope: { type: "factory", targetId: "" } } })} title={addAssignmentBlocked ? "Для добавления выберите новое немедленное назначение без периода и scope" : "Добавить вторую роль через PostgreSQL access-control owner"} type="button">Добавить назначение</button>
