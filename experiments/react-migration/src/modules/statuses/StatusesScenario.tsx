...
export type StatusesReactCommand =
...
export function StatusesScenario({ payload, onCommand, onNavigateSection, onRequestLegacy }: {
  ...
  const draftItem = draft ? model.items.find((item) => item.id === draft.itemId) : undefined;
  ...
  const deleteDraft = async () => {
    if (!draft || draft.isNew || !model.canDeleteCustom || draftItem?.isUserManaged !== true) return;
  ...
  return <ModulePage header={<ModuleHeader eyebrow="Мастер-данные" title="Статусы" badge={<span className="lab-badge" data-react-complete-marker>{model.canCreateEditCustom ? `React TS · custom-status create/edit${model.canDeleteCustom ? "/delete" : ""}` : "React TS · только чтение"}</span>} />} sidebar={sidebar}>
    ...
    {draft ? <Panel heading={<div className="panel-heading"><div><h2>{deletePending ? "Подтверждение удаления" : draft.isNew ? "Новый пользовательский статус" : "Редактирование пользовательского статуса"}</h2><p>Системные lifecycle-строки этим редактором не изменяются</p></div><ActionButton onClick={() => { if (deletePending) setDeletePending(false); else setDraft(null); clearCommandError(); }} variant="secondary">Отмена</ActionButton></div>}>
      {deletePending ? <DeleteConfirmation busy={saving} error={commandError} id="react-custom-status-delete-title" onCancel={() => setDeletePending(false)} onConfirm={() => { void deleteDraft(); }} title="Удалить пользовательский статус?">
        <p>Статус «{draft.name || "без названия"}» будет удалён только из пользовательского реестра.</p>
        <p>Системный lifecycle и существующие бизнес-объекты не изменяются: пользовательские статусы пока не являются их ссылочным контрактом.</p>
        ...
        <div className="react-nomenclature-editor-actions">
          {!draft.isNew ? <ActionButton disabled={!model.canDeleteCustom || draftItem?.isUserManaged !== true} onClick={() => { setDeletePending(true); clearCommandError(); }} title={draftItem?.isUserManaged ? "Удалить пользовательский статус" : "Системный статус удалить нельзя"} variant="danger">Удалить</ActionButton> : null}
          <button className="action action--primary" disabled={saving} type="submit">{saving ? "Сохранение…" : draft.isNew ? "Создать статус" : "Сохранить статус"}</button>
        </div>
      </form>
      }
    </Panel> : <>
