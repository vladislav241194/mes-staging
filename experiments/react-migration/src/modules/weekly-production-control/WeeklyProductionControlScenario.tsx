...
export function WeeklyProductionControlScenario({ payload }: { payload: unknown }) {
  ...
  return <ModulePage className="weekly-production-control-page" header={<ModuleHeader eyebrow="Планирование нагрузки" title="Контроль недели" badge={<StatusToken label={`Неделя · ${model.weekLabel}`} tone={model.canActivate ? "success" : "warning"} />} />}>
    <section className="workspace-main" data-weekly-production-control-react>
      {model.groups.length ? <Panel heading={<div><h2>План / факт по дням</h2><p>{model.days.length} дней · отклонение от плана дня</p></div>}>
