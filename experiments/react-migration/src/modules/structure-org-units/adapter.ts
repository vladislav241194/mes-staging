...
export interface StructureOrgUnit { id: string; name: string; code: string; kind: string; kindLabel: string; parentOrgUnitId: string; parentOrgUnitLabel: string; statusLabel: string; statusTone: "success" | "warning" }
export interface StructureOrgUnitsReadModel { orgUnits: StructureOrgUnit[]; counts: Record<StructureRegistryId, number> }
...
export function adaptStructureOrgUnits(payload: unknown): StructureOrgUnitsReadModel {
  ...
  const orgUnits = orgUnitRows.flatMap((entry): StructureOrgUnit[] => {
    ...
    return [{ id, name, code: text(entry.code) || "—", kind, kindLabel: KIND_LABELS[kind] || kind || "—", parentOrgUnitId, parentOrgUnitLabel: !parentOrgUnitId ? "—" : parent ? label(parent) : `${parentOrgUnitId} · связь не найдена`, statusLabel: entry.isActive === false ? "архив" : "активно", statusTone: entry.isActive === false ? "warning" : "success" }];
  ...
  return { orgUnits, counts: Object.fromEntries(STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => [definition.id, definition.id === "migrationDiagnostics" ? diagnosticsCount : rows(registries[definition.id]).length])) as Record<StructureRegistryId, number> };
}
