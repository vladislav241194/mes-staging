...
export interface StructureOrgUnitsReadModel { orgUnits: StructureOrgUnit[]; counts: Record<StructureRegistryId, StructureRegistryCount>; canCreateEdit: boolean; canArchive: boolean; canElevate?: boolean; writeUnavailableReason?: string }
...
export function adaptStructureOrgUnits(payload: unknown): StructureOrgUnitsReadModel {
  ...
  return { orgUnits, counts: Object.fromEntries(STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => [definition.id, definition.id === "migrationDiagnostics" ? diagnosticsCount : rows(registries[definition.id]).length])) as Record<StructureRegistryId, StructureRegistryCount>, canCreateEdit: capabilities.createEdit === true, canArchive: capabilities.archive === true, canElevate: capabilities.employeeElevation === true, writeUnavailableReason: text(capabilities.writeUnavailableReason) };
}
