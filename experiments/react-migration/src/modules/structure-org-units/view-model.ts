...
export function buildOrgUnitRegistryOptions(model: StructureOrgUnitsReadModel) { return STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => ({ ...definition, count: model.counts[definition.id], action: definition.id === "orgUnits" ? "orgUnits" as const : "legacy" as const })); }
