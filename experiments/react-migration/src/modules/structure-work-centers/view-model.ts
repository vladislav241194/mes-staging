...
export function buildWorkCenterRegistryOptions(model: StructureWorkCentersReadModel) { return STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => ({ ...definition, count: model.counts[definition.id], action: definition.id === "workCenters" ? "workCenters" as const : "legacy" as const })); }
