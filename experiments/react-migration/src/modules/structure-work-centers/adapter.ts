...
export interface StructureWorkCentersReadModel { workCenters: StructureWorkCenter[]; counts: Record<StructureRegistryId, StructureRegistryCount>; orgUnits: StructureWorkCenterOption[]; parentWorkCenters: StructureWorkCenterOption[]; canCreateEdit: boolean; canArchive: boolean; canElevate?: boolean; writeUnavailableReason?: string }
...
export function adaptStructureWorkCenters(payload: unknown): StructureWorkCentersReadModel {
  ...
  return { workCenters, counts: Object.fromEntries(STRUCTURE_REGISTRY_DEFINITIONS.map((definition) => [definition.id, definition.id === "migrationDiagnostics" ? diagnosticsCount : rows(registries[definition.id]).length])) as Record<StructureRegistryId, StructureRegistryCount>, orgUnits: options(orgUnits), parentWorkCenters: options(workCenterRows), canCreateEdit: capabilities.createEdit === true, canArchive: capabilities.archive === true, canElevate: capabilities.employeeElevation === true, writeUnavailableReason: text(capabilities.writeUnavailableReason) };
}
