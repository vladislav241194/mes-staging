...
import { buildComponentTypeFilters, filterComponentTypes, formatInteger, resolveVisibleComponentType, type ComponentTypeFilter } from "./view-model";

export function ComponentTypesScenario({ payload }: { payload: unknown }) {
  ...
  const header = <ModuleHeader eyebrow="Технологии" title="Типы компонентов" badge={<span className="lab-badge">React migration lab</span>} />;
  const sidebar = (
    ...
    <ModulePage header={header} sidebar={sidebar}>
      <Panel heading={<div className="panel-heading"><div><h2>Типы</h2><p>{formatRecordCount(visibleItems.length)} в выбранном семействе</p></div><ActionButton disabled title="Команды будут подключены после API checkpoint">Добавить тип</ActionButton></div>}>
        {visibleItems.length ? <TableWrap><table>
          ...
          <tbody>{visibleItems.map((item) => <SelectableRow key={item.id} onSelect={() => setSelectedId(item.id)} selected={selected?.id === item.id}>
            <td>{item.name}</td><td>{item.packageName}</td><td>{item.family}</td><td>{item.coefficient}</td><td>{formatInteger(item.placementsPerHour)}</td><td>{item.setupSeconds}</td><td>{item.defaultCount}</td><td><StatusToken label={item.statusLabel} tone={item.statusTone} /></td>
          </SelectableRow>)}</tbody>
      ...
      <DetailPanel
        ...
        fields={selected ? [
          ...
          { label: "Коэффициент", value: selected.coefficient },
          ...
          { label: "Количество по умолчанию", value: selected.defaultCount },
        ] : []}
        title={selected?.name}
      />
    </ModulePage>
