...
export function ModalOverlay({ children, className = "", eyebrow, footer, label, onClose, title }: { children: ReactNode; className?: string; eyebrow?: string; footer?: ReactNode; label: string; onClose(): void; title: string }) {
  const dialogRef = useRef<HTMLElement>(null);
  useEffect(() => {
    const previousFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    const dialog = dialogRef.current; const focusable = () => [...(dialog?.querySelectorAll<HTMLElement>('button:not([disabled]), [href], input:not([disabled]), textarea:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex="-1"])') || [])];
