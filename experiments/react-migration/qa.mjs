...
try {
  ...
  assert.match(commandParityMatrix.scenarios.find((scenario) => scenario.id === "shiftWorkOrders")?.nextVerticalScope || "", /fact\/correction is locally complete.*Shift Execution owner.*assignment and Workshop remain legacy.*Pilot write acceptance.*disposable cleanup/);
