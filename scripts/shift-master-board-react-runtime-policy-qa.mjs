...
assert.equal(module?.runtimeLegacyModelDependency, true, "typed UI cutover must not hide the transitional board-model dependency");
assert.match(hostSource, /canFallbackToLegacy:\s*\(activation\)\s*=>\s*activation\.accessMode !== "react"/);
assert.match(appSource, /command\.type === "move-lane"[\s\S]*moveShiftMasterBoardCardToLane/);
