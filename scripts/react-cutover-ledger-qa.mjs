...
assert.equal(ledger.implementationProgress, 82, "accelerated implementation progress must remain separate from evidence-weighted acceptance");
assert.equal(ledger.implementationProgressUpdatedAt, "2026-07-22", "implementation progress must name its checkpoint date");
assert.equal(ledger.implementationProgressBasis, "16/16 top-level routes have React UI: 7 complete, 7 partial, 2 explicit prototypes; strict Pilot acceptance remains separate at 50%.", "implementation progress must retain its auditable route basis");
