...
function order({ id, number, updatedAt, aggregateRevision }) {
  return {
    ...
    planning_start_date: "2026-07-21",
    source_revision: 3,
...
const compactOrders = orders.map(({ metadata, ...row }) => ({
  ...
  metadata: {
    ...
    planningStartDate: metadata.planningStartDate,
  },
}));
const operations = [{
  id: "route-old-step-1",
...
const sql = (strings, ...values) => {
  ...
  if (/SELECT wo\.\*/.test(query) && /WHERE wo\.id = \? OR wo\.number = \?/.test(query)) {
    const requestedId = String(values[0]);
    const matches = orders.filter((item) => String(item.id) === requestedId || String(item.number) === requestedId);
    if (/ORDER BY CASE WHEN wo\.id = \? THEN 0 ELSE 1 END, wo\.id/.test(query)) {
      matches.sort((left, right) => Number(String(left.id) !== requestedId) - Number(String(right.id) !== requestedId)
...
assert(result.items[0]?.planningStartDate === "2026-07-21", "compact list must map the canonical DATE column independently of stale metadata");
...
const aliasCollision = order({ id: "route-alias", number: "route-old", updatedAt: "2026-07-18T11:00:00.000Z", aggregateRevision: 12 });
orders.unshift(aliasCollision);
compactOrders.unshift({
  ...aliasCollision,
  metadata: {
    id: aliasCollision.metadata.id,
    name: aliasCollision.metadata.name,
    planningQuantity: aliasCollision.metadata.planningQuantity,
    planningStartDate: aliasCollision.metadata.planningStartDate,
  },
});
calls.length = 0;
transactions.length = 0;
const collisionBootstrap = await repository.listWorkbenchBootstrap("route-old");
const collisionDetailQuery = calls.find((call) => /SELECT wo\.\*/.test(call.query) && /WHERE wo\.id = \?/.test(call.query) && !/OR wo\.number/.test(call.query));
assert(collisionBootstrap.activeId === "route-old" && collisionBootstrap.item?.id === "route-old",
  "bootstrap must prefer an exact aggregate id over a newer row whose number alias collides");
assert(collisionDetailQuery?.values[0] === "route-old",
  "bootstrap detail, operations and slots must stay bound to the exact canonical aggregate");

calls.length = 0;
const collisionGet = await repository.get("route-old");
const collisionGetQuery = calls.find((call) => /WHERE wo\.id = \? OR wo\.number = \?/.test(call.query));
assert(collisionGet.item?.id === "route-old",
  "generic PostgreSQL get must resolve the same exact aggregate as workbench bootstrap and commands");
assert(/ORDER BY CASE WHEN wo\.id = \? THEN 0 ELSE 1 END, wo\.id\s+LIMIT 1/.test(collisionGetQuery?.query || ""),
  "generic get must encode exact-id-first ordering inside PostgreSQL");
orders.shift();
compactOrders.shift();

calls.length = 0;
transactions.length = 0;
const observedResult = await repository.readObservedWorkbenchBootstrap("WO-OLD", { contractVersion: 7 });
...
assert(observedResult.markerState?.snapshotGeneration === 14 && observedResult.markerState?.verifiedContractVersion === 7, "atomic bootstrap must expose the exact observed marker that admitted its result");
...
const pendingResult = await repository.readObservedWorkbenchBootstrap("WO-OLD", { contractVersion: 7 });
...
calls.length = 0;
markerRow = { ...observedMarker, verified_contract_version: 6 };
const outdatedContractResult = await repository.readObservedWorkbenchBootstrap("WO-OLD", { contractVersion: 7 });
assert(outdatedContractResult.admitted === false && calls.length === 2, "a v6 marker must fall through before reading v7 primary aggregate rows");

calls.length = 0;
markerRow = { ...observedMarker };
markerError = Object.assign(new Error("column does not exist"), { code: "42703" });
const schemaUnavailableResult = await repository.readObservedWorkbenchBootstrap("WO-OLD", { contractVersion: 7 });
markerError = null;
...
const lockContentionResult = await repository.readObservedWorkbenchBootstrap("WO-OLD", { contractVersion: 7 });
lockError = null;
