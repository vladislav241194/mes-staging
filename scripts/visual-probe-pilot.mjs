...
const defaultCssPath = join(projectRoot, "styles", "visual-overrides.dev.css");
const reportPath = join(projectRoot, "reports", "visual-probe-last-run.json");
...
async function main() {
  ...
  for (const targetDir of args.targetDirs) {
    ...
    rsyncArgs.push(args.file, `${args.remote}:${targetDir}/visual-overrides.dev.css`);
    steps.push(await runCommand(`rsync visual probe CSS -> ${targetDir}`, "rsync", rsyncArgs));
    if (steps.at(-1).code !== 0) throw new Error(steps.at(-1).stderr || `rsync failed: ${targetDir}`);
  }

  const cacheKey = `probe-${Date.now()}`;
  const url = `https://pilot.mes-line.ru/?module=${encodeURIComponent(args.module)}&qa-auth-bypass=1&visual_probe=1&__visual_probe_ts=${encodeURIComponent(cacheKey)}`;
  const report = {
    status: "ok",
    mode: "visual-probe",
    warning: "Pilot-only visual probe. No build, no version bump, no final publication.",
    module: args.module,
    file: args.file,
    targets: args.targetDirs.map((targetDir) => `${args.remote}:${targetDir}/visual-overrides.dev.css`),
    url,
    dryRun: args.dryRun,
  ...
  console.log("MES visual probe pilot");
