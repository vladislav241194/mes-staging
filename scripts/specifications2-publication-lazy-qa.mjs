...
expect(!appSource.includes('import { createSpecifications2RevisionsReadModel } from "./modules/domain_api/specifications2_revisions_read_model.ts";'), "Specifications 2.0 revision read API must not remain a static app import");
expect(!appSource.includes('import { createSpecifications2PublishCommands } from "./modules/domain_api/specifications2_publish_commands.ts";'), "Specifications 2.0 publish API must not remain a static app import");
...
expect(appSource.includes('import("./modules/domain_api/specifications2_revisions_read_model.ts")'), "Specifications 2.0 revision read API must load with its module");
expect(appSource.includes('import("./modules/domain_api/specifications2_work_order_commands.ts")'), "Specifications 2.0 work-order API must load with its production owner");
