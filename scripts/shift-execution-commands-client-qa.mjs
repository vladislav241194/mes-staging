...
const client = createShiftExecutionCommands({ fetchImpl: async (url, options) => {
  ...
  return { ok: true, status: 200, json: async () => ({ ok: true, capabilities: { assignmentCreationEnabled: true, primaryPostgres: true, schemaReady: true } }) };
} });

const capability = await client.refreshCapability();
assert(capability.ok && capability.enabled && capability.primaryPostgres && capability.schemaReady, "client must expose a fully ready server command capability");
