...
try {
  ...
  client.socket.addEventListener("message", (event) => {
    ...
    if (message.method === "Fetch.requestPaused") {
      ...
      if (requestUrl.pathname === "/api/v1/system-domains/capabilities" && ["positions", "orgUnits", "equipment"].includes(qaConfig.registryId)) { interceptedReads += 1; void fulfill(message.params.requestId, { ok: true, capabilities: { serverCommandsEnabled: true, serverCommandSurfaces: ["production-structure", "timesheet", "access-control"], ...(primaryAuthorityReady ? { consistency: { details: { authority: { mode: "postgres-primary" } } } } : {}) } }); }
      ...
      else if (requestUrl.pathname === "/api/v1/system-domains" && method === "PUT" && ["positions", "orgUnits", "equipment"].includes(qaConfig.registryId)) {
  ...
  if (["positions", "orgUnits", "equipment"].includes(qaConfig.registryId)) await evaluate(client, (key) => sessionStorage.setItem(key, "1"), SYSTEM_DOMAINS_PRIMARY_TOMBSTONE_KEY);
  ...
  if (qaConfig.registryId === "positions") {
    ...
    await waitForCondition(client, () => document.querySelectorAll('[data-system-domain-table="equipment"] [data-system-domain-row]').length === 7 && [...document.querySelectorAll('[data-system-domain-table="equipment"] [data-system-domain-row]')].some((row) => row.textContent?.includes("Принтер PostgreSQL QA обновлён")), { message: "legacy Equipment did not read back the React write" });
  }
