...
try {
  ...
  if (qaConfig.registryId === "positions") {
    ...
    assert(await evaluate(client, () => !document.querySelector('form.react-nomenclature-editor select[name="isActive"]')), "ordinary Position save must not expose lifecycle state");
    ...
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-positions-island] [data-ui-component="SelectableRow"]')].find((row) => row.textContent?.includes(id))?.click(), created.id);
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Восстановить"), { message: "archived Position reactivation action did not become available" });
    ...
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление"), { message: "Position reactivation confirmation was not explicit" });
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-positions-island] [data-ui-component="SelectableRow"]')].find((row) => !row.textContent?.includes(id))?.click(), created.id);
    assert(await evaluate(client, () => ![...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление")), "Position reactivation confirmation must not follow another selected row");
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-positions-island] [data-ui-component="SelectableRow"]')].find((row) => row.textContent?.includes(id))?.click(), created.id);
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление"), { message: "Position-specific reactivation confirmation was not retained" });
    ...
    await waitForCondition(client, () => [...document.querySelectorAll('[data-react-structure-positions-island] [data-ui-component="SelectableRow"]')].some((row) => row.textContent?.includes("Инженер PostgreSQL QA обновлён") && row.textContent?.includes("активно")), { message: "reactivated Position did not return through PostgreSQL read model", timeoutMs: 15_000 });
    assert(apiRevision === 5 && successfulWrites === 4 && putAttempts === 5, "Position reactivation must advance exactly one revision");
    const reactivated = apiDomains.registries.positions.find((position) => position.id === created.id);
    assert(reactivated?.isActive === true && reactivated?.archivedAt === "", "Position reactivation did not clear the lifecycle archive marker");
    assert(reactivated?.serverOnlyMarker === "position-hidden-field" && reactivated?.orgUnitId === references.orgUnitId && reactivated?.workCenterId === references.workCenterId && reactivated?.defaultScheduleTemplateId === references.scheduleTemplateId, "Position reactivation changed hidden or reference fields");
    ...
    await waitForCondition(client, () => document.querySelectorAll('[data-system-domain-table="positions"] [data-system-domain-row]').length === 50 && [...document.querySelectorAll('[data-system-domain-table="positions"] [data-system-domain-row]')].some((row) => row.textContent?.includes("Инженер PostgreSQL QA обновлён") && row.textContent?.includes("активно")), { message: "legacy Positions did not read back the React reactivation" });
    ...
    assert(await evaluate(client, () => !document.querySelector('form.react-nomenclature-editor select[name="isActive"]')), "ordinary Work Center save must not expose lifecycle state");
    ...
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-work-centers-island] [data-ui-component="SelectableRow"]')].find((row) => row.textContent?.includes(id))?.click(), created.id);
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Восстановить"), { message: "archived Work Center reactivation action did not become available" });
    await evaluate(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].find((button) => button.textContent?.trim() === "Восстановить")?.click());
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление"), { message: "Work Center reactivation confirmation was not explicit" });
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-work-centers-island] [data-ui-component="SelectableRow"]')].find((row) => !row.textContent?.includes(id))?.click(), created.id);
    assert(await evaluate(client, () => ![...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление")), "Work Center reactivation confirmation must not follow another selected row");
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-work-centers-island] [data-ui-component="SelectableRow"]')].find((row) => row.textContent?.includes(id))?.click(), created.id);
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление"), { message: "Work Center-specific reactivation confirmation was not retained" });
    await evaluate(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].find((button) => button.textContent?.trim() === "Подтвердить восстановление")?.click());
    await waitForCondition(client, () => [...document.querySelectorAll('[data-react-structure-work-centers-island] [data-ui-component="SelectableRow"]')].some((row) => { const cells = row.querySelectorAll("td"); return row.textContent?.includes("Рабочий центр PostgreSQL QA обновлён") && cells[4]?.textContent?.trim() === "активно"; }) && [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Архивировать" && !button.disabled), { message: "reactivated Work Center did not return through PostgreSQL read model", timeoutMs: 15_000 });
    assert(apiRevision === 5 && successfulWrites === 4 && putAttempts === 5, "Work Center reactivation must advance exactly one revision");
    const reactivated = apiDomains.registries.workCenters.find((center) => center.id === created.id);
    assert(reactivated?.isActive === true && reactivated?.archivedAt === "", "Work Center reactivation did not clear the lifecycle archive marker");
    assert(reactivated?.serverOnlyMarker === "work-center-hidden-field" && reactivated?.orgUnitId === references.orgUnitId && reactivated?.parentWorkCenterId === "" && reactivated?.participatesInPlanning === true && reactivated?.showInGantt === true, "Work Center reactivation changed hidden, hierarchy or Planning/Gantt fields");
    ...
    assert(await evaluate(client, () => !document.querySelector('form.react-nomenclature-editor select[name="isActive"]')), "ordinary Equipment save must not expose lifecycle state");
    ...
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-equipment-island] [data-ui-component="SelectableRow"]')].find((row) => row.textContent?.includes(id))?.click(), created.id);
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Восстановить"), { message: "archived Equipment reactivation action did not become available" });
    await evaluate(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].find((button) => button.textContent?.trim() === "Восстановить")?.click());
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление"), { message: "Equipment reactivation confirmation was not explicit" });
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-equipment-island] [data-ui-component="SelectableRow"]')].find((row) => !row.textContent?.includes(id))?.click(), created.id);
    assert(await evaluate(client, () => ![...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление")), "Equipment reactivation confirmation must not follow another selected row");
    await evaluate(client, (id) => [...document.querySelectorAll('[data-react-structure-equipment-island] [data-ui-component="SelectableRow"]')].find((row) => row.textContent?.includes(id))?.click(), created.id);
    await waitForCondition(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].some((button) => button.textContent?.trim() === "Подтвердить восстановление"), { message: "Equipment-specific reactivation confirmation was not retained" });
    await evaluate(client, () => [...document.querySelectorAll('[data-ui-component="ActionButton"]')].find((button) => button.textContent?.trim() === "Подтвердить восстановление")?.click());
    await waitForCondition(client, () => [...document.querySelectorAll('[data-react-structure-equipment-island] [data-ui-component="SelectableRow"]')].some((row) => row.textContent?.includes("Принтер PostgreSQL QA обновлён") && row.textContent?.includes("активно")), { message: "reactivated Equipment did not return through PostgreSQL read model", timeoutMs: 15_000 });
    assert(apiRevision === 5 && successfulWrites === 4 && putAttempts === 5, "Equipment reactivation must advance exactly one revision");
    const reactivated = apiDomains.registries.equipment.find((equipment) => equipment.id === created.id);
    assert(reactivated?.isActive === true && reactivated?.archivedAt === "", "Equipment reactivation did not clear the lifecycle archive marker");
    assert(reactivated?.serverOnlyMarker === "equipment-hidden-field" && reactivated?.orgUnitId === references.orgUnitId && reactivated?.workCenterId === references.workCenterId && reactivated?.scheduleTemplateId === references.scheduleTemplateId && reactivated?.quantity === 4, "Equipment reactivation changed hidden, reference or quantity fields");
    ...
    await waitForCondition(client, () => document.querySelectorAll('[data-system-domain-table="equipment"] [data-system-domain-row]').length === 7 && [...document.querySelectorAll('[data-system-domain-table="equipment"] [data-system-domain-row]')].some((row) => row.textContent?.includes("Принтер PostgreSQL QA обновлён") && row.textContent?.includes("активно")), { message: "legacy Equipment did not read back the React reactivation" });
