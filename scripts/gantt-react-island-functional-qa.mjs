...
try {
  ...
  const react = await evaluate(client, () => { const target = document.querySelector("[data-react-gantt-island]"); const canvas = document.querySelector('[data-ui-component="GanttCanvas"]'); const slots = [...document.querySelectorAll('[data-ui-component="GanttSlot"]')]; slots.find((slot) => !slot.classList.contains("is-aggregate"))?.click(); return { rows: document.querySelectorAll("[data-row-id]").length, slots: slots.length, metrics: document.querySelectorAll('[data-ui-component="MetricCard"]').length, width: canvas?.getBoundingClientRect().width || 0, commitMs: Number(target?.getAttribute("data-react-island-commit-ms")), source: document.body.innerText.includes("PostgreSQL projection"), detail: document.querySelector(".gantt-react-detail h2")?.textContent?.trim(), overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth }; });
  ...
  assert(react.commitMs < 2000 && react.detail && !react.overflow, `Gantt React style/telemetry failed: ${JSON.stringify(react)}`);
  await evaluate(client, () => document.querySelector(".gantt-react-detail .action")?.click());
  ...
  console.log("- default legacy, PostgreSQL projection gate, local selection, edit fallback and zero API writes: pass");
} catch (error) {
