...
expect(source.includes('["gantt", "shiftMasterBoard", "shiftWorkOrders"].includes(ui?.activeModule)') && /const applied = await hydratePlanningRuntimeProjection\(\);/.test(source), "Direct Gantt and Shift Execution consumers must request the PostgreSQL runtime projection before the compatibility snapshot.");
