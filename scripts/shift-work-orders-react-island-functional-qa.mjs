...
try {
  ...
  console.log("Shift Work Orders React production-shell functional QA: OK"); console.log(`- parity: ${react.documents} work order, ${react.operations} operation, ${react.rows.length} assignment, 8 columns; first commit ${state.commitMs.toFixed(2)} ms`); console.log(`- PostgreSQL projections, lazy SZN/package previews, host print callback, zero Shift Execution writes, ${sharedStateWrites} compatibility write(s) intercepted, unchanged state and clean console: pass`);
