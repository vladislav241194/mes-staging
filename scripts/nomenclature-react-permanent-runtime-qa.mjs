...
assert.match(appSource, /async function getNomenclatureReactWriteDecisionForCommand[^]*nomenclatureServerCapabilitiesState\.status\s*===\s*["']loading["'][^]*await\s*\(nomenclatureServerCapabilitiesPromise\s*\|\|\s*ensureNomenclatureServerCapabilities\(\{\s*force:\s*true\s*\}\)\)[^]*getNomenclatureReactWriteDecision\(action\)/, "a command must await an in-flight capability refresh before its final fail-closed decision");
assert.match(appHostSource, /command\.type\s*===\s*["']delete["'][^]*await\s+getNomenclatureReactWriteDecisionForCommand\(["']delete["']\)/, "delete dispatch must independently await and recheck its server capability");
