...
const enabled = createSpecifications2AttachmentCommands({ fetchImpl: async (url, options) => {
  ...
  if (url.endsWith("/capabilities")) return { ok: true, status: 200, json: async () => ({ attachmentUploadEnabled: true }) };
...
const disabledCalls = [];
const disabled = createSpecifications2AttachmentCommands({ fetchImpl: async (url) => { disabledCalls.push(url); return { ok: true, status: 200, json: async () => ({ attachmentUploadEnabled: false }) }; } });
