...
assert(systemDomainsTimesheetDropIn.includes("MES_SYSTEM_DOMAINS_SERVER_COMMAND_SURFACES=production-structure,timesheet"), "Timesheet rollout artifact must keep an explicit cumulative surface list");
...
assert(systemDomainsCommandSurfaceActivation.includes("Usage: activate-system-domains-command-surfaces.sh --through=production-structure|timesheet"), "Staged command-surface rollout must advertise the two reviewed targets");
const productionOnlyGate = systemDomainsCommandSurfaceActivation.indexOf('access-control)');
const rolloutLockEntry = systemDomainsCommandSurfaceActivation.indexOf('with-authority-rollout-lock.sh');
assert(productionOnlyGate >= 0 && rolloutLockEntry > productionOnlyGate, "Access Control activation must fail before the rollout lock or any service mutation");
assert(systemDomainsCommandSurfaceActivation.includes("Access Control server writes remain disabled until their server authorization and delta invariants are implemented."), "Unreviewed Access Control writes must fail closed with an explicit reason");
...
assert(systemDomainsCommandSurfaceRollback.includes("primarySuspendReady") && systemDomainsCommandSurfaceRollback.includes('authority === "postgres-primary"') && systemDomainsCommandSurfaceRollback.includes("retirementEligible"), "Staged command-surface rollback must permit tombstone-preserving PostgreSQL-primary downshifts");
...
assert(systemDomainsPrimaryCommandRecovery.includes('EXPECTED_CSV="production-structure"') && systemDomainsPrimaryCommandRecovery.includes("Timesheet is activated separately"), "PostgreSQL-primary recovery must restore Production Structure before the separate Timesheet stage");
